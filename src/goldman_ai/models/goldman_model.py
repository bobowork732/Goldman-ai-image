...
class GoldmanAIModel:
    ...
    def model_identifier(self) -> str:
        """Return a stable model identifier for telemetry."""
    ...
    def _coerce_pil(self, image: Any) -> Any:
        """Convert path-like image references to PIL image when needed."""
    ...
    def generate_image_to_image(self, *, prompt: str, image: Any, strength: float = 0.8, **overrides: Any) -> list[Any]:
        ...
        if self._backend == "diffusers" and self._img2img_pipeline is not None:
            try:
                init_image = self._coerce_pil(image)
        ...
        image_name = Path(image).name if isinstance(image, str) else "uploaded-image"
        return [self._placeholder_image(f"image-to-image: {image_name}") for _ in range(settings["sample_count"])]

    def edit_add_object(self, *, image: Any, object_prompt: str, mask_or_box: Any, **overrides: Any) -> list[Any]:
        ...
        if self._backend == "diffusers" and self._inpaint_pipeline is not None:
            try:
                init_image = self._coerce_pil(image)
                mask_image = self._coerce_pil(mask_or_box)
    ...
    def edit_remove_object(self, *, image: Any, mask_or_box: Any, **overrides: Any) -> list[Any]:
    ...
    def edit_restyle(self, *, image: Any, style_prompt: str, **overrides: Any) -> list[Any]:
